...
import frc.robot.commands.DemoAiming;
...
public class RobotContainer {
    ...
    public DemoAiming demoAimingCommand;
    ...
    private RobotContainer() {
        ...
        demoAimingCommand = new DemoAiming(turret, shooter, () -> overrideTurret, () -> RPM.of(shooterSpeed));
    ...
    private void configureBindings() {
        ...
        commandDriver2.leftTrigger().whileTrue(new RepeatCommand(new RunRoller(intake, false)));
        ...
        commandDriver2
                ...
                .whileTrue(new RepeatCommand(new ConditionalCommand(
                        new Shoot(indexer, intake, true), new StopShoot(indexer, intake), () -> isReadyToShoot())))
        ...
        new Trigger(() -> Math.abs(ControllerUtil.applySimpleDeadband(hidDriver2.getRightX(), 0.5)) > 0)
                .onTrue(new InstantCommand(() -> {
                    overrideTurret = true;
                }));
        new Trigger(() -> Math.abs(ControllerUtil.applySimpleDeadband(hidDriver2.getLeftY(), 0.2)) > 0)
                .whileTrue(new RepeatCommand(new InstantCommand(() -> {
                    shooterSpeed = MathUtil.clamp(
                            shooterSpeed - 8 * hidDriver2.getLeftY(), 0, ShooterConstants.maxRealSpeed.in(RPM));
                })));
        commandDriver2
                .rightStick()
                .onTrue(new InstantCommand(() -> {
